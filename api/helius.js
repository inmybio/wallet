...
function tokenAmount(transfer) {
  return Number(transfer?.rawTokenAmount?.tokenAmount ?? transfer?.tokenAmount ?? 0);
}
...
async function buyValueUsd(event) {
  ...
  const stableInput = (swap?.tokenInputs ?? []).find((input) => QUOTE_MINTS.has(input.mint) && input.mint !== SOL_MINT);
  if (stableInput) {
...
async function tokenDetails(mint) {
  ...
  const pair = (await response.json())?.pairs?.sort((a, b) => (b.liquidity?.usd ?? 0) - (a.liquidity?.usd ?? 0))[0];
  if (!pair) return {};
  return {
...
async function researchOnX({ mint, token, buyUsd }) {
  ...
  if (!response.ok) throw new Error(`xAI research failed (${response.status})`);
...
async function postDiscord({ mint, token, buyUsd, narrative }) {
  const lines = [
    `**Wallet buy detected — $${token.symbol ?? "TOKEN"}**`,
    `Bought: **$${buyUsd.toFixed(2)}**`,
    `CA: \`${mint}\``,
    token.marketCap ? `MC: **$${Math.round(token.marketCap).toLocaleString()}**` : null,
    token.liquidityUsd ? `Liquidity: **$${Math.round(token.liquidityUsd).toLocaleString()}**` : null,
    "",
    narrative,
    "",
    `[Open X search](https://x.com/search?q=${encodeURIComponent(mint)}&src=typed_query)${token.url ? ` · [DexScreener](${token.url})` : ""}`,
  ].filter(Boolean).join("\n");
  const response = await fetch(env("DISCORD_WEBHOOK_URL"), {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ content: lines, allowed_mentions: { parse: [] } }),
  });
  if (!response.ok) throw new Error(`Discord delivery failed (${response.status})`);
}

export default async function handler(req, res) {
  if (req.method === "GET") return res.status(200).json({ ok: true, service: "wallet-narrative-tracker" });
  if (req.method !== "POST") return res.status(405).json({ error: "Method not allowed" });
  if (req.headers.authorization !== `Bearer ${env("HELIUS_WEBHOOK_SECRET")}`) return res.status(401).json({ error: "Unauthorized" });
  ...
  for (const event of events) {
    if (event.type !== "SWAP" || !event.accountData?.some((account) => account.account === wallet)) continue;
    const bought = findBoughtToken(event, wallet);
    if (!bought) continue;
    const buyUsd = await buyValueUsd(event).catch(() => 0);
    if (buyUsd < minimum) continue;
    const token = await tokenDetails(bought.mint);
    const narrative = await researchOnX({ mint: bought.mint, token, buyUsd });
    await postDiscord({ mint: bought.mint, token, buyUsd, narrative });
    results.push({ mint: bought.mint, symbol: token.symbol, buyUsd });
  }
